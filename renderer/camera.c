...
{
	struct GLCameraObject *cam_obj = &ctx->camera.gl_camera_object;
	glBindBuffer(GL_UNIFORM_BUFFER, cam_obj->UBO);
	glBufferSubData(GL_UNIFORM_BUFFER, 0, sizeof(cam_obj->ubo_data),
			&cam_obj->ubo_data);
...
void init_camera_ubo(gx_ctx *ctx)
{
	...
	ctx->camera.gl_camera_object = gl_camera_obj;
...
void update_camera_projection(gx_ctx *ctx)
{
	...
	for (size_t i = 0; i < ctx->glob_resources.gl_camera_objs.count; i++) {
		struct GLCameraObject *cam_obj =
			da_at(&ctx->glob_resources.gl_camera_objs, i);
