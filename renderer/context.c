...
gx_ctx* gx_ctx_init(int frame_width, int frame_height) {
        ...
        ctx->glob_resources.VAOs            = array_create(sizeof(u32));
...
void gx_ctx_drop(gx_ctx* ctx) {
        ...
        for (int i = 0; i < ctx->glob_resources.VAOs.count; i++) {
                glDeleteVertexArrays(1, (unsigned int*) array_at(
                                                &ctx->glob_resources.VAOs, i));
        }
        ...
        terminate_glfw();
        array_delete(&ctx->glob_resources.VAOs);
